...
| `shipyard_neo_profile` | 沙盒 profile。保留默认值 `python-default`，或者留空时，Bay 会自动选择最合适的 profile；手动填写时请使用 Bay 返回的 profile id。 |
...
- 如果你不确定该填哪个 profile，直接保留 `python-default`，让 Bay 自动选择即可。
